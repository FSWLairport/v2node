...
func NewServer(ctx context.Context, config *ServerConfig) (*Server, error) {
	...
	certMode := strings.ToLower(strings.TrimSpace(config.CertMode))
	switch certMode {
	case "", "none", "self":
		// Empty and none retain the legacy SATLS behavior, which generated a
		// self-signed certificate when no externally managed certificate existed.
		if err := ensureSelfSignedCertificate(s.serverName, config.CertFile, config.KeyFile, "main"); err != nil {
			return nil, err
		}
		if s.upServerName != "" && s.upServerName != s.serverName {
			if err := ensureSelfSignedCertificate(s.upServerName, s.upCertFile, s.upKeyFile, "up"); err != nil {
				return nil, err
			}
		}
		if s.downServerName != "" && s.downServerName != s.serverName {
			if err := ensureSelfSignedCertificate(s.downServerName, s.downCertFile, s.downKeyFile, "down"); err != nil {
				return nil, err
			}
		}
	case "file", "dns", "http", "remote":
		// DNS/HTTP ACME modes are legacy externally managed certificate modes,
		// and remote mode has already written the panel-supplied PEM to these
		// paths; like file mode, the certificate must exist before we start.
		if config.CertFile == "" || config.KeyFile == "" || !fileExists(config.CertFile) || !fileExists(config.KeyFile) {
			return nil, stderrs.New("satls: configured certificate file does not exist")
		}
	default:
